...
// these variables are used for tracking FX
bool activatedAnyFX = false;
int activeFX = 0;

// these variables are for tracking the LFOs
const int numLFOs = 4;
LFO lfos[numLFOs];
...
void loop() {
  ...
  // Update potentiometers... and the color palettes, which use them 
  if (millis() > nextPotUpdateTime) {
    updatePots();
    nextPotUpdateTime = millis() + potIntervalMS;

    if (potsUpdated || previousActivePalette != activePalette || activatedAnyFX) {
      ...
      // These are FX that modify the color palette, so we call them here
      if (activatedAnyFX) {
        switch(activeFX) {
          case 7:
            invert();
      ...
      // Because we're doing this here, we don't need to do it in the palettes.ino functions
      // unlike in previous example projects!
      display.swap(false, true);
    }
  }

  // Do any "Pre FX" before drawing the new frame
  if (activatedAnyFX == true) {
    switch(activeFX) {
      case 0:
        cutFade();
        break;
  ...
  if (showDebug == false) {
    switch(activeLayer) {
      ...
      // layers.ino file!
      // For example, orbitingCircles() is under case 0, so that will run
      // when you press button 0.
      
      case 0:
        slidingCirclesDifferentWaveforms();
        break;
      case 1:
        animatedSpark();
        break;
  ...
  // Do any "Post FX" after drawing the new frame
  if (activatedAnyFX == true) {
    switch(activeFX) {
      case 4:
        mirrorHorizontally();
